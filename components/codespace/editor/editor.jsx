...
export default function Editor() {
    ...
    useEffect(() => {
        ...
        for (var i = 0; i < tabs.current.length; i++) {
            ...
            tab.className =
                "tab flex h-min py-2 px-5 hover:brightness-150 bg-[var(--bg-2)] text-[var(--fg-1)] cursor-pointer";
            ...
            tab.addEventListener("click", function (e) {
                ...
                for (var i = 0; i < allTabs.length; i++) {
                    ...
                    allTabs[i].classList.remove("border-[var(--fg-1)]");
                }
        ...
        document
            .getElementById("menubar")
            .firstChild.classList.add("border-[var(--fg-1)]");
    ...
    return (
        <>
            <div
                ...
                className="h-10 flex overflow-y-hidden font-mono text-sm bg-[var(--bg-2)] overflow-x-scroll"
            ></div>
            <div id="wrapper" className="w-full h-full flex font-mono">
                <textarea
                    id="line-numbers"
                    className="min-w-[5ch] max-w-[9ch] text-right flex-none pr-[1ch] border-none outline-none rounded-bl-lg overflow-hidden resize-none bg-[var(--bg-2)] text-[var(--fg-1)]"
                    cols="1"
                    rows="10"
                    readOnly
                ></textarea>
                <textarea
                    ...
                    autoCorrect="off"
                    spellCheck="false"
                    className="w-full pl-2 border-none outline-none whitespace-pre rounded-br-lg overflow-wrap-normal resize-none bg-[var(--bg-1)] text-[var(--fg-1)]"
                ></textarea>
            </div>
        </>
    );
