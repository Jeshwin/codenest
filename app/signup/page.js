...
export default function Page() {
    ...
    return (
        <body className="overflow-hidden h-screen bg-[var(--dark-bg-3)]">
            ...
            >
                <div
                    className="w-1/4 min-w-fit flex flex-col gap-y-5 items-center
                    p-8 rounded-xl bg-[var(--dark-bg-2)] text-[var(--dark-fg-1)]"
                >
                    ...
                    {SSOOptions.map((sso, index) => {
                        return (
                            <button
                                key={index}
                                className={`w-full py-3 rounded-lg grid place-content-center font-sans ${sso.bgColor} hover:brightness-90 active:scale-90 transition duration-150`}
                            >
